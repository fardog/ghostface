...
function cli(argv, _next) {
  ...
  var helpMessage = [
    ...
    , '  -t --timeout         Milliseconds to wait for output before stopping execution. Default ' + DEFAULT_TIMEOUT
  ...
  ;[].concat(options.string, options.numeric).forEach(function(i) {
...
function createCheckPhantom(_phantomPath) {
  ...
  return function checkPhantom(_next) {
    ...
    exec(phantomPath + ' --version', function(err, stdout) {
      if (err) {
